...
set.seed(123)
setwd("~/SI")
DATA_1.1=read_ods("SPEI_12_2.6.ods",col_names = TRUE,row_names = TRUE,as_tibble = FALSE)
DATA_1.1 %<>% na.omit()
# rownames(DATA_1.1)=as.character(DATA_1.1$TIME)
# DATA_1.1 %<>% select(-TIME) 
DATA_1.1$Dakhla <- as.numeric(DATA_1.1$Dakhla)
print(head(DATA_1.1))
```
...
```{r}
setwd("~/SI")
DATA_2.1=read_ods("SPI_12_2.6.ods",col_names = TRUE,row_names = TRUE,as_tibble = FALSE)
# DATA %<>% na.omit()
# rownames(DATA_2.1)=as.character(DATA_2.1$TIME)
# DATA_2.1 %<>% select(-TIME) 
DATA_2.1$Dakhla <- as.numeric(DATA_2.1$Dakhla)
print(head(DATA_2.1))
```
...
colnames(DF_combined) <- paste0("E", 1, "_2.2")
for (i in c(2:nombre_clusters)) {
  assign(paste0("E",i,"_2.2"),round(kmeans_resultats_2.2$centers[i,], 3))
  x=get(paste0("E",i,"_2.2"))
  temp_df <- data.frame(x)
  colnames(temp_df) <- paste0("E", i, "_2.2") 
...
for (i in c(1:nombre_clusters)) {
  ...
  # Define the function dynamically and assign it to a named list element
  assign(paste0("f_2.", i), function(x) sum((x - get(paste0("E", i, "_2.2")))^2))
  
  # Use apply to find the minimum and get the corresponding name (ETALON)
  etalon_name[i] <- names(which.min(apply(MD_2.2, 1, get(paste0("f_2.", i)))))
  
  # Assign the result to a dynamic variable (ETALON_i.1)
  # assign(paste0("ETALON_", i, "_2.2"), etalon_name[i])
