...
set.seed(123)
setwd("~/SI")
DATA_1.1=read_ods("SPEI_9_REA.ods",col_names = TRUE,row_names = TRUE,as_tibble = FALSE)
# DATA %<>% na.omit()
...
CITY=c()
DF_1.1=DATA_1.1
# DATE = seq.Date(from = as.Date("1950-12-01"), length.out = nrow(DATA_1.1), by = "month")
# DF_1.1$DATE <- DATE
...
for ( i in (1:nombre_clusters)) {
  ...
  cat("le pourcentage de la classe ",i," est :",TEXT,"\n")
}
```
...
DATE = seq.Date(from = as.Date("1950-12-01"), length.out = length(E1_1.1), by = "month")
DF_2.1$DATE <- DATE
# Loop to create and store each plot in the list
for (i in 1:nombre_clusters) {
  # Dynamically get the column name and ETALON
  column_name <- paste0("E", i, "_2.1")
  # etalon_name <- get(paste0("ETALON_", i, "_1.1"))
